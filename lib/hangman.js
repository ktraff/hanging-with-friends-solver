	        ...
	        var c = token.charAt(index);
                if (c === '*') {
                    ...
                    for (var key in curNode) {
                        if (index === (token.length - 1) && this.isValidLetter(key, token) && this.isEndOfWord(curNode[key])) {
		        ...
		        if (index === token.length - 1 && this.isEndOfWord(curNode[c])) {
		       	    this.trigger('added', new Hangman.Models.Match({ word: curWord + c }));
		        }
		        else {
    ...
    Hangman.Views.App = Backbone.View.extend({
        ...
        initialize: function () {
            ...
            this.matchesView = new Hangman.Views.Matches({
                el: '#matches',
                collection: new Hangman.Collections.Matches()
            });
            this.wordFinder.on('added', this.matchesView.addMatch);
        },
